...
app.command("/tour-de-france-current-stage", async ({ ack, respond }) => {
  ...
  try {
    ...
    // Find stage happening today
    let currentStage = stages.find((s) => s.date && s.date.startsWith(todayStr));

    // Fallback: If today isn't a race day, pick the next upcoming stage or the latest one
    if (!currentStage) {
      const now = new Date();
...
// Specific Stage information command
...
// General Classification (GC) command
